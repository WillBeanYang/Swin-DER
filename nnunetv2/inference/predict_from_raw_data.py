...
from SRUNet.predict_from_multiscales_data import nnUNetWaveletPredictor
import nnunetv2
...
class nnUNetPredictor(object):
    def __init__(self,
        ...
        self.subband_mode = subband_mode
...
#                                                           num_input_channels, configuration_manager.patch_size,
#                                                           self.upsample, enable_deep_supervision=False)
        network = trainer_class.build_network_architecture(plans_manager, dataset_json, configuration_manager,
                                                           num_input_channels, configuration_manager.patch_size,
                                                           self.upsample, self.subband_mode, self.use_wavelets,
                                                           self.input_downsample, enable_deep_supervision=False)
    ...
    def predict_from_data_iterator(self,
        ...
        # clear device cache
        empty_cache(self.network.device)
        return ret
    ...
    def predict_sliding_window_return_logits(self, input_image: torch.Tensor) \
        ...
        with torch.no_grad():
            with torch.autocast(self.device.type, enabled=True) if self.device.type == 'cuda' else dummy_context():
                ...
                if self.is_ddp:
                    results_device = self.network.device if self.perform_everything_on_gpu else torch.device('cpu')
                else:
                ...
                for sl in tqdm(slicers, disable=not self.allow_tqdm):
                    workon = data[sl][None]
                    if self.is_ddp:
                        workon = workon.to(self.network.device, non_blocking=False)
                    else:
...
def predict_entry_point():
    ...
    parser.add_argument('--upsample', type=str, default='transconv',
                        help="The upsampling method employed by networks.")
    parser.add_argument("--wavelet_function", type=str, default='haar',
                        help="Type of wavelet basis functions.")
    parser.add_argument("--subband_mode", type=str, default='approximate', choices=["approximate", "all"],
    ...
    if args.device == 'cpu':
        ...
        device = torch.device('mps')
    if args.use_wavelets or args.input_downsample:
        predictor = nnUNetWaveletPredictor(subband_mode=args.subband_mode,
        ...
        predictor = nnUNetPredictor(subband_mode=args.subband_mode,
        					        upsample=args.upsample,
                                    tile_step_size=args.step_size,
                                    use_gaussian=True,
                                    use_mirroring=not args.disable_tta,
